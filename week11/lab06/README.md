...
| 리스크 | 영향도 | 가능성 | 대응책 |
|---|---|---|---|
| MySQL 환경 차이로 팀원 PC 빌드 실패 | 高 | 中 | H2 프로필 기본 + Docker 옵션 검토 |
| 발표 직전 git 충돌 | 中 | 高 | feature 브랜치 + 매주 PR 머지 |
| 파일 업로드 보안 사고 | 高 | 低 | UUID 파일명 + Path Traversal 방어 + 확장자 검증 |
| 팀원 1명 결석 시 작업 지연 | 中 | 中 | 짝 작업(pair) · Must 기능 우선 |
